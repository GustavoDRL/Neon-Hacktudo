  ...
  void loop() {
    ...
    //motors_control(linear_speed*multiplicador, angular_speed* multiplicador2); 
      float multiplic_curva = map(PS4.LStickY(), -154, 155, 0.5, 2.5);
      motors_control(1.8*inv*PS4.LStickY(), inv*(-1)*PS4.RStickX()/multiplic_curva);
      ...
      if (PS4.Cross()) { 
       ...
       if(PS4.R1()){
          ...
          angle = map(PS4.R2Value(), 0, 255, 50, 55);        
          servo.write(angle);
          Serial.println(angle);
       }   
    } 
